...
success_y = train_data[['task_success', 'pick_success', 'place_success']].values
total_y = train_data[['task_total', 'pick_total', 'place_total']].values
y = success_y / total_y
...
for i in range(2):
    ...
    model.fit(X, y[:, i], sample_weight=weights[:, i])
    models.append(model)
